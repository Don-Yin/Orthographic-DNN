...
def n_subloop(words_a, words_b, function=hamming_distance, threshold=1, n_jobs=-1):
    """Calculate distance from each word in word_a to each word in word_b."""
    func = partial(calc_dist, words_b=words_b, threshold=threshold, function=function)
...
def n_n(words, reference=None, n=1, function=hamming_distance, n_jobs=-1):
    """
    ...
    if np.any([len(reference) < x for x in n]):
        raise ValueError("The number of reference words you have is lower than" " or equal to n. Please lower n.")
    if len(set(words)) != len(words):
        raise ValueError(
            "There are duplicates in your dataset. Please remove" " these, as they will make your estimates unreliable"
        )

    vals = n_subloop(words, reference, function, max(n), n_jobs)
